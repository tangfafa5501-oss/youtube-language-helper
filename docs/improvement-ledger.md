...
| 11 | 完整短句保留 canonical 时间，不因低于 2000ms 被硬合并或延长 | 单元测试 + 生产集成 |
...
| 15 | Side Panel 派生视图明确输出 `data-display-mode=phrases` | 浏览器门禁 |
| 16 | 每个 DOM 字幕节点都具有可解析时间 | 浏览器门禁 |
| 17 | 点击字幕等待 `seeked` 并验证误差不超过 80ms | 单元测试 + 集成测试 + 20/20 浏览器门禁 |
| 18 | 主字幕和第二字幕独立，副轨不改主轨时间 | 单元测试 + 生产集成 |
| 19 | B站默认英语主轨配网站手工简体中文轨 | 单元测试 + 生产集成 |
| 20 | SPA、分 P、快速切轨和多标签页使用会话隔离 | 生产集成 |
| 21 | Auto/Manual 明确分离；上/下一句强制 Manual，句尾暂停，播放只推进一条 | 单元测试 + 生产集成 + 20/20 浏览器门禁 |
...
| 28 | 快速连击立即取消旧 seek；切轨、SPA、断连和播放器替换销毁旧监听器 | 单元测试 + 生产集成 + 浏览器断连反例 |
| 29 | 播放控制不改变 YouTube 原生 CC 或 B站原生 text track 状态 | 20/20 浏览器门禁 |

最新基线：82/82 单元测试、61/61 生产 bundle 集成测试、TypeScript 类型检查、生产构建和 20/20 测试浏览器播放闭环通过。后续数字只能由新一轮完整命令输出刷新。
