...
import { PORT, emptyState, type State, type Track } from '../lib/protocol';
...
export default defineContentScript({
  ...
  main(ctx) {
    ...
    let state: State = emptyState(), locationKey = '', controller: AbortController | null = null;
    ...
    const playback = () => {
      const video = videoElement(); if (!video || video.readyState === 0) return;
      const message = { type: 'playback-state', currentTimeMs: Math.round(video.currentTime * 1000), playing: !video.paused, rate: video.playbackRate };
    ...
    async function refresh() {
      ...
      if (nextKey === locationKey && state.video) return;
      ...
      try {
        ...
        if (request.signal.aborted || locationKey !== nextKey) return;
        ...
        if (locationKey === nextKey) { state = { ...emptyState(), status: 'error', message: error instanceof Error ? error.message : 'B 站视频读取失败' }; publish(); }
      }
    }
    async function seek(message: Record<string, unknown>, port: Browser.runtime.Port) {
      const info = state.video;
      if (!info || !clients.has(port) || message.videoId !== info.videoId || message.session !== info.session || message.trackId !== state.trackId) return;
      ...
      const video = videoElement(); if (startMs === null || startMs === undefined || !video || video.readyState === 0) return;
      const mode: Mode = message.playMode === 'loop' || message.playMode === 'all' ? message.playMode : 'single';
      active = typeof endMs === 'number' && endMs > startMs ? { owner: port, startMs, endMs, mode, generation: ++generation, looping: false } : null;
      video.currentTime = startMs / 1000;
      try { await video.play(); port.postMessage({ type: 'playback', message: `已定位至 ${(startMs / 1000).toFixed(3)} 秒并请求播放` }); }
      catch { port.postMessage({ type: 'playback', message: '已定位，播放被浏览器拦截' }); }
    }
    browser.runtime.onConnect.addListener(port => {
      ...
      port.onMessage.addListener(message => {
        ...
        const video = videoElement(); if (!video || message.videoId !== state.video?.videoId || message.session !== state.video?.session) return;
        if (message.type === 'playback-toggle') { if (video.paused) void video.play(); else video.pause(); }
        if (message.type === 'playback-rate' && typeof message.rate === 'number' && [.75,.8,.9,1].includes(message.rate)) video.playbackRate = message.rate;
        if (message.type === 'playback-mode' && (message.mode === 'single' || message.mode === 'loop' || message.mode === 'all') && active?.owner === port) {
          active = { ...active, mode: message.mode, generation: ++generation, looping: false };
        }
      });
    ...
    ctx.setInterval(() => {
      ...
      if (!item || !video || item.mode === 'all' || item.looping || video.paused || video.currentTime * 1000 < item.endMs - 20) return;
      ...
      setTimeout(() => { if (active?.generation !== token || active.mode !== 'loop') return; void video.play().finally(() => { if (active?.generation === token) active.looping = false; }); }, 500);
    }, 50);
    ctx.onInvalidated(() => { controller?.abort(); for (const port of clients) port.disconnect(); clients.clear(); });
  },
});
