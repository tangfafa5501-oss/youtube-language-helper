...
YouTube 的自然语段优先使用自动轨词级时间。整轨至少 80% 词可对齐才采用该轨；某一自然语段只有全部词命中时才标为 `youtube-word`，局部错词、漏词或重复词只让该段使用 Supadata cue 内的确定性估算并标为 `youtube-estimated`。文本分段先于时间分配，时间层不能跨句号改写 SBD 结果；相邻时间不得倒退或重叠。B站仍只使用网站 cue 级 `from/to`，不沿用该 YouTube 估算路径。
