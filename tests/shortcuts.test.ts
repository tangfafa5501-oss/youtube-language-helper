...
test('native keyboard decoder maps learning shortcuts without a focus library', () => {
  for (const [code, action] of [['KeyA', 'previous'], ['KeyS', 'replay'], ['KeyD', 'next'], ['KeyE', 'shadowing'],
    ['Space', 'play'], ['KeyK', 'play']]) assert.equal(shortcutAction({ code }), action);
...
test('button focus permits A/S/D/E but leaves Space to native button activation', () => {
  ...
  assert.equal(shortcutAction({ code: 'Space', target: button }), null);
});
